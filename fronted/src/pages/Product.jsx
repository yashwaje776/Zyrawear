                 ...
                 <div className='flex gap-2'>
                  {
                    productData.sizes.map((items,idx)=>(
                      <button key={idx} className={`bg-gray-200 w-16 h-10 py-2 px-4 rounded-md border-1 cursor-pointer ${size === items ? "border-yellow-500" : "border-transparent"}`} onClick={() => setsize(items)}> {items}</button>
                    ))
                  }
                 </div>
              </div>
              <button className='bg-black text-white mt-8 py-3 px-8 text-sm cursor-pointer' onClick={()=>addProTocart(productData._id,size)}>ADD TO CART</button>
